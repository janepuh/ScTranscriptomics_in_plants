...
Or you can use the data we already downloaded [via this link](https://drive.google.com/drive/folders/11hHUQ3nabTj230urqKnTXPCwoqWhZ8Zc?usp=sharing).

---
#Step 0. Course material download

Preparatory steps:
1. You need to clone/download [GitHub repo](https://github.com/VictoriaVMironova/ScTranscriptomics_in_plants) with all the course materials (including this lecture).
2.  Inside the resulting folder, create a folder `Data/GSM5097888` and save the counts data in there.
3. open `ScTranscriptomics_inPlants.Rproj ` project </br>
...
1. You can use the file `_packages.R` to install all the packages needed in a course.</br>

2. open the script `CP1_from_counts_to_clusters.R` for today's practice.

3. Load the libraries using the script. Today we need only Seurat and ggplot2 packages:
...
- This presentation is generated from .Rmd file which you can also find the repository.
- The presentation visualizes the output of the code chunks that `CP1_from_counts_to_clusters.R` contains.
- We will go through the code one by line together. 
...
- Explore gene expression for other known marker genes in Arabidopsis leaf (find them using google).
